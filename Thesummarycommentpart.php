...
//Målet är att kunna läsa av vilken sammanfattnig
<?php
	$host = "localhost";
...
<?php
	...
	if($pdo)
	{
		foreach ($pdo->query("select comments.date, comments.title, comments.text, comments.author from comments where id = comment_id") as $row)
		{
			echo "<p>{$row['date']}.{$row['title']}.{$row['text']}.{$row['author']}</p>";		
		}
	}
